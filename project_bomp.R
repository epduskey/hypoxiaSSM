# Run hypoxia model for BOMP

# Created: March 3, 2021
# Last modified: December 19, 2022 by EPD
...
species_mat = sp_mat(rmax = c(4.0e+08,9.6e+08,2.1e+11,1.9e+10))
...
species_params(params_test)$z0 = c(0.06, 0.22, 0.30, 0.24)
species_params(params_test)$gamma = c(9.1e-11, 4.3e-11, 2.3e-11, 3.0e-11)
...
# Fit occupancy
species_params(params_tune)$z0 = c(0.06, 0.22, 0.30, 0.24)
species_params(params_tune)$gamma = c(9.1e-11, 4.3e-11, 2.3e-11, 3.0e-11)
par_occ = c(0.8, 0.1)
...
params_test = params_tune
species_params(params_test)$R_max = c(1.1e+08,4.4e+08,5.0e+11,2.3e+10)
sim_test = project(params_test, t_max = t_tune, effort = effort_hypoxia)
plotBiomassObservedVsModel(sim_test)
plotBiomass(sim_test)

# Fit biomass and growth
par_bg = c(log10(c(1.1e+08,4.4e+08,5.0e+11,2.3e+10)), log10(species_params(params_tune)$gamma))
...
params_time = oxy_sensitivity(params_time, 
				U_hab = c(occ.optim$par[1], 10^5, -10^5, -10^5),
				a_hab = c(occ.optim$par[2], 0, 0, 0),
				U_crit = c(tg.optim.cod.phys$par[1], uc_flounder, 10^5, 10^5),
				a_crit = c(tg.optim.cod.phys$par[2], ac_flounder, 0, 0),
				U_met = c(tg.optim.cod.phys$par[3], um_flounder, 10^5, 10^5),
				a_met = c(tg.optim.cod.phys$par[4], am_flounder, -10^5, -10^5),
...
params_time = oxy_sensitivity(params_time, 
				U_hab = c(occ.optim$par[1], 10^5, -10^5, -10^5),
				a_hab = c(occ.optim$par[2], 0, 0, 0),
				U_crit = c(tg.optim.cod.phys$par[1], tg.optim.flounder.phys$par[1], 10^5, 10^5),
				a_crit = c(tg.optim.cod.phys$par[2], tg.optim.flounder.phys$par[2], 0, 0),
				U_met = c(tg.optim.cod.phys$par[3], tg.optim.flounder.phys$par[3], 10^5, 10^5),
				a_met = c(tg.optim.cod.phys$par[4], tg.optim.flounder.phys$par[4], -10^5, -10^5),
				z_mort = c(zm_cod, zm_flounder, 0, 0),
				b_mort = c(bm_cod, bm_flounder, 0, 0))
...
benthic.optim = optimParallel(par = par_tg_benthic, params = params_time, benthic_growth, yr = yr_tg, t = t_tune, effort = effort_time, lower = c(0,-10), method = "L-BFGS-B", control = list(factr=1e11), parallel = list(cl=cl,loginfo=T))
stopCluster(cl)
