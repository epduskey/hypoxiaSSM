...
getHypoxiaDiet = function(sim, t, proportion = T) {
	...
	diet[,,1:(no_sp+1+1)] = sweep(sweep(diet[,,1:(no_sp+1+1),drop = FALSE], c(1,3), inter, "*"), c(1,2), params@other_params$rate_scale[t,,]*params@search_vol, "*")
...
# My energy (g/year) available for reproduction and growth function
getHypoxiaErag = function(sim, t) {
	...
	# Get encounter and feeding level
	encounter = getEncounter(params, n, n_pp, n_other, t = t)
	feeding_level = getFeedingLevel(params, n, n_pp, n_other, time_range = t)
	
	# Calculate energy available for reproduction and growth
	erag = hypoxiaEReproAndGrowth(params, n, n_pp, n_other, t, encounter, feeding_level)
	dimnames(erag) = dimnames(params@metab)
